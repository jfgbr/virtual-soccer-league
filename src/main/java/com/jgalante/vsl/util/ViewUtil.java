...
public class ViewUtil {
	...
	public static List<SelectItem> wrapInSelectItems(
		...
		for (BaseEntity e : entities) {
			results.add(new SelectItem(e.getId(), e.getTextoExibicao()));
		}
		return results;
	}
	...
	public static void redirect(String url) {
		try {
			...
			ctx.getExternalContext().redirect(ctx.getExternalContext()
					.encodeActionURL(ctx.getApplication().getViewHandler().getActionURL(ctx, url)));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
